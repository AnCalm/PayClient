...
namespace ChargeInterface.MBJ
{
    public class PayAndQueryMBJ
    {
        public const string submitUrl = "http://api.281559955.com/Api/Pay";
        public const string queryUrl = "http://api.281559955.com/Api/QueryOrder";
        public const string key = "n6ln6ikodhiuavy";
        public const string merchantID = "10026";
        ...
        public string SubmitOrder(Order order, string str = null)
        {
            ...
            if (string.IsNullOrEmpty(str))
                str = setPostDate(order, "http://116.62.44.48/NotifyFromMBJ.aspx", AreaValueType.MBJ);

            WriteLog.Write("方法:SubmitOrder，订单号：" + order.OrderInsideID + " MBJ 提交参数:" + str.ToString(), LogPathFile.Recharge.ToString());

            string result = PostAndGet.HttpPostString(submitUrl, str.ToString(), ref coockie);

            WriteLog.Write("方法:SubmitOrder，订单号：" + order.OrderInsideID + " MBJ 提交返回:" + result, LogPathFile.Recharge.ToString());
