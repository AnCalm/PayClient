...
{
    public class QueryBaiYou : IQuery
    {
        public Order Query(Order order)
        {
            int num = 0;

            string MerchantID = PayAndQueryBaiYou.merchantID;
            string key = PayAndQueryBaiYou.key;
            string QueryUrl = PayAndQueryBaiYou.queryUrl;
            ...
            while (num < 5)
            {
                ...
                WriteLog.Write("方法:Query，订单号：" + order.OrderInsideID + " BaiYou 订单查询参数:" + str.ToString(), LogPathFile.Recharge.ToString());

                string result = PostAndGet.HttpPostString(QueryUrl, str.ToString(), ref coockie);

                WriteLog.Write("方法:Query，订单号：" + order.OrderInsideID + " BaiYou 订单查询:" + result, LogPathFile.Recharge.ToString());
                ...
                string BaiYouStateInfo = Regex.Match(result, @"<state-info>(.*?)</state-info>").Groups[1].Value;


                if (state.Equals("101"))
                {
                    order.RechargeStatus = (int)OrderRechargeStatus.successful;
                    order.RechargeMsg = BaiYouStateInfo + "-" + BaiYouOrderId;
                    break;
                }
                ...
                {
                    int status = getOrderStatus(BaiYouStateInfo, "601");
                ...
                else
                {
                    ...
                    order.RechargeMsg = EnumService.GetDescription(OrderRechargeStatus.suspicious) + "-" + BaiYouOrderId;
                    break;
                }
