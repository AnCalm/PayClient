...
{
    public class Query99vip : IQuery
    {
        public Order Query(Order order)
        {
            string MerchantID = PayAndQuery99vip.merchantID;
            string key = PayAndQuery99vip.key;
            string QueryUrl = PayAndQuery99vip.queryUrl;
            ...
            WriteLog.Write("方法:Query，订单号：" + order.OrderInsideID + " 99vip 订单查询参数:" + str.ToString(), LogPathFile.Recharge.ToString());

            string result = PostAndGet.HttpPostString(QueryUrl, str.ToString(), ref coockie);

            WriteLog.Write("方法:Query，订单号：" + order.OrderInsideID + " 99vip 订单查询:" + result, LogPathFile.Recharge.ToString());
