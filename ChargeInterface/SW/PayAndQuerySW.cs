...
{
    public class PayAndQuerySW
    {
        public const string submitUrl = "http://api.hphy888.com/Api/Pay";
        public const string QueryUrl = "http://api.hphy888.com/Api/QueryOrder";
        public const string key = "y7y8hihweqcb408";
        ...
        public string SubmitOrder(Order order, string str = null)
        {
            ...
            WriteLog.Write("方法:SubmitOrder，订单号：" + order.OrderInsideID + " SW 提交参数:" + str.ToString(), LogPathFile.Recharge.ToString());

            string result = PostAndGet.HttpPostString(submitUrl, str.ToString(), ref coockie);

            WriteLog.Write("方法:SubmitOrder，订单号：" + order.OrderInsideID + " SW 提交返回:" + result, LogPathFile.Recharge.ToString());
        ...
        string setPostDate(Order order)
        {
            ...
            string ResponseUrl = System.Web.HttpUtility.UrlEncode("http://116.62.44.48/NotifyFromSW.aspx", Encoding.UTF8); ;	//接收异步通知订单状态的Url
       ...
       public  string getGameValue(string gameName,int productID=0)
        {
            ...
            //List<ProductInfo> lst = SQLProductInfo.GetProductInfosByProductID(productID);
            //string GameValue = lst.Where(n => n.GameName == gameName).Select(n=>n.GameValue).FirstOrDefault();
