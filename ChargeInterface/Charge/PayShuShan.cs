...
{
    class PayShuShan : ICharge
    {
        public Order Charge(Order order)
        {
            try
            {
                PayAndQueryShuShan payShuShan = new PayAndQueryShuShan();

                string result = payShuShan.SubmitOrder(order);
                ...
                if (state.Equals("101"))
                {
                    ...
                    order.RechargeMsg = stateInfo + "-" + ShuShanOrderId;
                    order.IsNotify = true;
            ...
            {
                WriteLog.Write("PayShuShan: 订单号：" + order.OrderInsideID + ",异常信息：" + ex.Message, LogPathFile.Exception.ToString());
            }
            finally
            {
